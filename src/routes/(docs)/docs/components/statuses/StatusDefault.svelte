<script>
	import { Status } from '@ubeac/svelte'
</script>

<Status color="primary">Primary</Status>
<Status color="secondary">Secondary</Status>
<Status color="success">Success</Status>
<Status color="warning">Warning</Status>
<Status color="danger">Danger</Status>
<Status color="info">Info</Status>
<Status color="light">Light</Status>
<Status color="dark">Dark</Status>
<Status color="blue">Blue</Status>
<Status color="azure">Azure</Status>
<Status color="indigo">Indigo</Status>
<Status color="purple">Purple</Status>
<Status color="pink">Pink</Status>
<Status color="red">Red</Status>
<Status color="orange">Orange</Status>
<Status color="yellow">Yellow</Status>
<Status color="lime">Lime</Status>
<Status color="green">Green</Status>
<Status color="teal">Teal</Status>
<Status color="cyan">Cyan</Status>
