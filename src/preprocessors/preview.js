		...
		markup({ content, filename }) {
			...
			walk(ast.html, {
				enter(node) {
					...
					const srcAttribute = node.attributes.find((/** @type {any} */ node) => node.name === 'src')
					...
					const { markup: previewMarkup, script: previewScript, style: previewStyle } = spliteSections(sourceCode)
